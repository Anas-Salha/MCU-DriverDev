...
int main(void){
	...
	while(1){
		...
		if(GPIO_InPinRead(GPIOA, USER_BUTTON) == 1){
			...
			I2C_MasterTX(&i2cMaster, &cmd_code, 1, SLAVE_ADDR, I2C_SR_EN);
			//2. Receive length
			I2C_MasterRX(&i2cMaster, &Len, 1, SLAVE_ADDR, I2C_SR_EN);
			//3. Send DATA_REQUEST
			cmd_code = CMD_DATA_REQ;
			I2C_MasterTX(&i2cMaster, &cmd_code, 1, SLAVE_ADDR, I2C_SR_EN);
			//4. Receive data
			I2C_MasterRX(&i2cMaster, Data, Len, SLAVE_ADDR, I2C_SR_DI);
			//uint8_t x = 0;
		}
	}
